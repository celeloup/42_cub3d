...
/*   Updated: 2020/04/17 17:35:26 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			raycasting(t_window *win, t_vector_d player)
{
	...
	while (x < win->set.res_x)
	{
		...
		ray.y = win->set.player_dir_y + win->scene.plane_y * \
			(2 * x / (double)win->set.res_x - 1);
		if (shoot_ray(win, player, ray, &perp_wall_dist) == 0)
			texture = ray.x < 0 ? win->set.text_no : win->set.text_so;
		else
			texture = ray.y < 0 ? win->set.text_we : win->set.text_ea;
		line_height = (int)(win->set.res_y / perp_wall_dist);
		draw_limits = get_draw_limits(win, line_height);
		w_texture = get_wall_texture(win, texture, perp_wall_dist, ray);
		texture_pos = (draw_limits.x - win->set.res_y / 2 + line_height / 2) \
			* (1.0 * texture.height / line_height);
		...
		while (draw_limits.x < draw_limits.y)
		{
			...
			texture_pos += 1.0 * texture.height / line_height;
